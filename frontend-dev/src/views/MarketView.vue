...
const SORTS = computed(() => [
  ...
  { value: 'price', label: t('低价') },
  { value: 'orders', label: t('热门') },
])
...
async function doCall() {
  ...
  if (wallet.wallet.balance < est) {
    ui.toast({
      type: 'warn',
      title: t('余额不足'),
      desc: t('本次调用预计约 {e}，当前余额 {b}', { e: money(est), b: money(wallet.wallet.balance) }),
      action: { label: t('去充值'), onClick: () => router.push('/wallet') },
    })
    return
  }
...
const catOptions = computed(() => market.CATS.map((c) => ({ value: c.value, label: t(c.label) })))
</script>
...
<template>
  <div class="page">
    <div class="page-head">
      <div>
        <h1 class="page-title">{{ t('Agent 市场') }}</h1>
        <p class="page-sub">{{ t('跨团队调用各类 Agent，按 token 按次计费，用多少付多少') }}</p>
      </div>
      <div class="head-actions">
        <AppButton icon="plus" @click="router.push('/market/publish')">{{ t('上架 Agent') }}</AppButton>
      </div>
    </div>
    ...
    <div class="toolbar">
      <div class="toolbar__search">
        ...
        <AppInput v-model="market.query" :placeholder="t('搜索名称、厂商或标签')" />
      </div>
      ...
      <AppCard v-for="m in market.filtered" :key="m.id" :hover="true" class="m-card" @click="router.push('/market/' + m.id)">
        <div class="m-card__top">
          ...
          <div class="m-card__info">
            <div class="m-card__name">
              {{ m.name }}
              <AppTag v-if="m.featured" variant="brand" class="tag-feat">{{ t('推荐') }}</AppTag>
            </div>
            <div class="m-card__vendor">{{ m.vendor }}</div>
          </div>
          <span class="m-card__dot" :title="m.online ? t('在线') : t('维护中')" :class="{ off: !m.online }" />
        </div>
        ...
        <div class="m-card__foot">
          ...
          <div class="m-card__meta">
            ...
            <span class="m-card__orders num">{{ fmtNum(m.orders) }} {{ t('次调用') }}</span>
          </div>
        </div>
    ...
    >
      <div v-if="callTarget" class="call">
        <div class="call__est">
          ...
          <span>{{ t('本次预估价 {e} · 余额 {b}', { e: money(estCost(callTarget)), b: money(wallet.wallet.balance) }) }}</span>
        </div>
        <textarea v-model="prompt" class="call__prompt" rows="4" :placeholder="t('描述你要完成的任务')" />
      </div>
      <template #footer>
        <AppButton variant="ghost" @click="callTarget = null">{{ t('取消') }}</AppButton>
